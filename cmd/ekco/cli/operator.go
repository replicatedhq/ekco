...
func OperatorCmd(v *viper.Viper) *cobra.Command {
	...
	cmd.Flags().Bool("reconcile_ceph_csi_resources", true, "Set Ceph CSI provisioner and plugin resources to their recommendations once the cluster is scaled to three nodes")
	...
	_ = cmd.Flags().MarkDeprecated("contour_cert_namespace", "use contour_namespace instead")
	_ = cmd.Flags().MarkHidden("contour_cert_namespace")
